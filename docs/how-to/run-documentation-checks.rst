...
The Sphinx Stack comes with several tests and checks that you can (and should!) run on
your documentation before committing and pushing changes:
...
<https://docs.npmjs.com/downloading-and-installing-node-js-and-npm>`__ for details.
Then:
...
If you need to exempt single words or sections, follow the process in
:ref:`vale_exemptions`. 
...
The Sphinx Stack checks standards and consistency in Markdown files with the Markdown
lint check. To check your Markdown files, run:
...
Vale can run against individual files, directories, or globs. To check a specific
target, run:
...
You can use wildcards to lint globs of files. For example, to run against all Markdown
files within a directory, you'd run:
...
The class process isn't be necessary for Markdown, as Vale has an expanded scope for
ignoring Markdown content by default. Also, the `.. class::` directive for reST does not
need to encapsulate content, it applies to the next logical block (which can be another
directive or even a paragraph of content). The class option should only be used when
other options aren't suitable.

.. admonition:: Hyphens and spaces
    :class: note

    The checks may flag terms that contain hyphens or spaces.

    For example, "Juju 3" wasn't ignored by default, and `needed to be added to be
    excepted with an upstream rule
    <https://github.com/canonical/documentation-style-guide/blob/a6f530b07d774bee67dd79d146ae5bbedc9ddef1/styles/Canonical/013-Spell-out-numbers-below-10.yml#L15>`_.
